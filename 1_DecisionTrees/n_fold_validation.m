...
function [results, confusionMatrix] = n_fold_validation(examples, classifications,...
    ...
    % upper and lower portitions of the matrix, and then recombine
        ...
        test_classes = classifications((i-1)*partition_size + 1:i*partition_size, :);
        ...
        outcomes = classify(emotion_decision_trees,...
            test_examples, strategy);
        
        % Concatinate the classifications
        classifcation_results((i-1)*partition_size+1:i*partition_size,:)...
            = outcomes;
        
        confusionMatrix{i} = confusion_matrix(test_classes,outcomes, 6);
    ...
    test_classes = classifications(lower_bound + 1 : length(classifications), :);
    ...
    outcomes = classify(emotion_decision_trees,  test_examples, strategy);
    
    %Concatinate the classifications
    classifcation_results(((n - 1)*partition_size+1) : len,:) =...
        outcomes;
    
    confusionMatrix{n} = confusion_matrix(test_classes,outcomes, 6);
